...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton(' Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/Moviesrequest_chat"),
                    InlineKeyboardButton('🎬 Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/Moviesrequest_info')
                ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = ' file_name  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = ' file_name ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
